...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [{ title: "LoveSpace — пространство для вашей пары" }],
  }),
  component: Landing,
});
...
function Landing() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-background text-foreground selection:bg-primary/20">
      {/* Анимированный фон */}
      <div className="pointer-events-none absolute left-1/2 top-0 h-200 w-full -translate-x-1/2 bg-[radial-gradient(circle_at_center,rgba(var(--color-primary-rgb),0.16)_0,transparent_70%)]" />
      ...
      <main className="relative z-10 mx-auto max-w-5xl px-6 pt-20 pb-32 text-center md:pt-32">
        <h1 className="font-display text-6xl font-black leading-[0.9] tracking-tighter sm:text-8xl md:text-9xl">
          Станьте <br />
          <span className="bg-linear-to-b from-primary to-indigo-500 bg-clip-text text-transparent">
            Ближе
          </span>
        </h1>
        ...
        <p className="mx-auto mt-10 max-w-xl text-lg font-medium leading-relaxed text-muted-foreground md:text-xl">
          LoveSpace — это ваш цифровой дом. Ежедневные ритуалы, общие воспоминания и бесконечные
          идеи для свиданий.
        </p>
        ...
        <div className="mt-32 grid grid-cols-1 gap-6 text-left sm:grid-cols-3">
          {[
            ...
            return (
              <div
                key={f.title}
                className="rounded-[40px] border border-border bg-card p-10 shadow-sm"
              >
